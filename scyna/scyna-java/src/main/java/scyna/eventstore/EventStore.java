...
public class EventStore<D extends Message> {
    ...
    private PreparedStatement undoLockingQuery;
    private Parser<D> parser;
    private Message.Builder builder;
    private Map<String, IProjection> projections = new HashMap<String, IProjection>();

    @SuppressWarnings("unchecked")
    public Class<D> init(String tableName) throws Exception {
        this.tableName = tableName;
        var session = Engine.DB().getSession();

        Class<D> cls = (Class<D>)((ParameterizedType) getClass().getGenericSuperclass()).getActualTypeArguments()[0];
        ...
        undoLockingQuery = session
                .prepare("UPDATE " + tableName + " SET state=0 WHERE id=? AND version=? IF state=1");
    ...
    boolean lockingLongRow(Object id, long version) {
        try {
            ...
            if (locked.toInstant().plusSeconds(5).isBefore(Instant.now())) {
                return Engine.DB().getSession().execute(undoLockingQuery.bind(id, version)).one().getBool("[applied]");
            }
            return false;
        } catch (Exception e) {
    ...
    boolean syncRow(Object id, long version) {
        try {
            var row = Engine.DB().getSession().execute(getSyncRowQuery.bind(id, (Long)version)).one();
            if (row == null)
                return false;
