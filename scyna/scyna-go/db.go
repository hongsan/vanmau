...
func (db *db) CounterBatch() *batch {
	return &batch{
		batch: db.Session.NewBatch(gocql.CounterBatch),
		db:    db,
	}
}
...
func (db *db) QueryManyPaging(query string, args ...interface{}) gocql.Scanner {
	const pageSize = 256
	return db.QueryManyPagingWithSize(query, pageSize, args...)
}

func (db *db) QueryManyPagingWithSize(query string, pageSize int, args ...interface{}) gocql.Scanner {
	return db.Session.Query(query, args...).
		WithContext(context.Background()).
		PageSize(pageSize).
		Iter().Scanner()
