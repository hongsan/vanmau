...
func (es *eventStream) start() {
	assureStreamReady(es.sender, es.receiver)
	sub, err := JetStream.PullSubscribe(es.sender+".>", es.receiver, nats.BindStream(es.sender))

	if err != nil {
		ctx := Context{ID: 0}
		ctx.PublishAlertToDeveloper("Module "+module+" cannot pull subscribe event stream",
			module+" pull subscribe event stream"+" - Sender: "+es.sender+" - Receiver: "+es.receiver+" throw error "+err.Error(),
			scyna_proto.AlertSeverity_ERROR)
...
func assureStreamReady(sender, receiver string) {
	if _, err := JetStream.StreamInfo(sender); err != nil {
		if _, err := JetStream.AddStream(&nats.StreamConfig{
			Name:     sender,
			Subjects: []string{sender + ".>"},
			Storage:  nats.FileStorage,
			Replicas: 2,
			MaxAge:   time.Hour * 24 * 7, //keep for a week
		}); err != nil {
			ctx := Context{ID: 0}
			ctx.PublishAlertToDeveloper("Module "+module+" cannot add consumer to JetStream",
				"No stream `"+sender+"` "+err.Error(),
				scyna_proto.AlertSeverity_ERROR)
	...
	if _, err := JetStream.ConsumerInfo(sender, receiver); err != nil {
		if _, err := JetStream.AddConsumer(sender, &nats.ConsumerConfig{
			...
			ctx := Context{ID: 0}
			ctx.PublishAlertToDeveloper("Module "+module+" cannot add consumer to JetStream",
				"No consumer `"+module+"` for stream `"+sender+"`",
				scyna_proto.AlertSeverity_ERROR)
