...
func (e *EventStore[T]) ReadModel(id any) (*Model[T], Error) {
	...
	var ret = scyna_utils.NewMessageForType[T]()
	...
	if err := DB.QueryOne("SELECT version, data FROM "+e.Table+
		...
		if err == gocql.ErrNotFound {
			return &Model[T]{
				ID:      id,
				Version: 0,
				Data:    scyna_utils.NewMessageForType[T](),
				store:   e,
			}, nil
		}
		return nil, SERVER_ERROR
...
func (e *EventStore[T]) lockLongLockingRow(id any, version int64) bool {
	...
	if locked.Add(time.Second * 5).Before(time.Now()) {
		if applied, _ := DB.Apply("UPDATE "+e.Table+" SET locked=? WHERE id=? AND version=? IF state=? AND locked=?",
			time.Now(), id, version, 1, locked); applied {
			return true
		}
	}
	return false
